...
def random_fake_inf0(self):  # Thêm self
    """Tạo thông báo giả mạo ngẫu nhiên."""
    action = random.choice(["disconnect", "reload"])
    if action == "disconnect":
        for char in "Error....":
            self.write_output(char)
            time.sleep(0.1)  # Điều chỉnh thời gian delay
        disconnect_attacker()

    elif action == "reload":
        message = "Yêu cầu khởi động lại host để áp dụng thay đổi. Khởi động lại trong 10 giây..."
        for char in message:
            self.write_output(char)
            time.sleep(0.05)  # Điều chỉnh thời gian delay

        for _ in range(7):
            for char in [".", ".", ".", "   "]:
                self.write_output(char)
                time.sleep(0.5)
                self.write_output("\b\b\b\b\b")  # Xóa 5 ký tự
        ...
        for _ in range(3):
            for char in [".", "..", "   "]:
                self.write_output(char)
                time.sleep(0.3)
                self.write_output("\b\b\b")  # Xóa 3 ký tự
        disconnect_attacker()

def options_fake_info(self,namespace):
    ...
    if namespace == "graphics":
        self.write_output("GPU Device:\r\n"
  "- Name: NVIDIA Tesla T4\r\n"
  "- Device ID: 1B06:10DE\r\n"
  "- Total Memory: 15109 MB\r")
  ...
  - datastore2 (500GB): 15% full"""
    # Thêm các trường hợp cụ thể khác ở đây
        ...
        for _ in range(3):
            self.write_output(f"  - Thuộc tính {random.randint(1, 10)}: Giá trị {random.randint(100, 1000)}\r\n")
...
def handle_vm_process(self,filesystem, *args):
    ...
    if len(args) == 0:
        ...
        if fake_vms:
            for vm in fake_vms:
                self.write_output(f"World ID: {vm['World ID']} Name: {vm['Name']}\r\n")
        else:
            self.write_output("No VMs found.\r\n")
    elif args[0] == "kill":
        if len(args) < 2:
            self.write_output("Lỗi: Thiếu World ID của máy ảo.\r\n")
            disconnect_attacker()
        else:
            world_id = int(args[1])
            ...
            if matching_vms:
                ...
                self.write_output(f"Không tìm thấy máy ảo với World ID: {world_id}\r\n")
    else:
...
def handle_network(self,filesystem, *args):
    ...
    if len(args) == 0:
        ...
        if handler:
            ...
            self.write_output(f"Lỗi: Chưa triển khai handler cho '{args[0]}'.\r\n")
    else:
    ...
    if len(args) == 0:
        ...
        if fake_vms:
            for vm in fake_vms:
                ...
                self.write_output("-" * 20)  # Dòng phân cách
        else:
            self.write_output("No VMs found.\r\n")
    elif args[0] == "port":
        if len(args) < 2:
            ...
            matching_vms = [vm for vm in fake_vms if vm["World ID"] == world_id]
            if matching_vms:
                self.write_output(f"Thông tin port cho VM với World ID {world_id}:\r\n")
...
def handle_unimplemented_namespace(self,filesystem, namespace, *args):
    ...
    if action == "random":
        print(random_fake_inf0(self))
    elif action == "options":
        option_info = options_fake_info(self,namespace)
        print(option_info)
...
def handle_namespace(self,filesystem, namespace, *args):
    ...
    if namespace in handlers:
        handlers[namespace](self,filesystem, *args)  # Truyền filesystem cho handler
    else:
...
def handle_command(self,filesystem, command):
    ...
    parts = command.split()
    # if len(parts) < 2:
    #     self.write_output("Lỗi: Thiếu namespace và command.\r\n")
    #     return
...
class ESXiEsxcliCommand(cmd.SimpleCommand):
    ...
    def run(self):
        if len(self.args) == 0 or self.args[0] in ["-?", "--help"]:
            ...
            self.write_output("Lỗi: Thiếu thông tin đầu vào.\r\n")
